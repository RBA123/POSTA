...
const ProfileScreen: React.FC = () => {
  ...
  const initials = `${profile.firstName?.charAt(0) || "U"}${
    profile.lastName?.charAt(0) || ""
  }`;
  const fullName = `${profile.firstName || "Usuario"} ${
    profile.lastName || ""
  }`.trim();
      ...
      <SafeAreaView className="flex-1" edges={["top"]}>
        ...
        <ScrollView className="px-4 py-6" showsVerticalScrollIndicator={false}>
          <View className="gap-2">
            ...
            <Pressable
              onPress={() =>
                navigation.navigate("NotificationsHistory" as never)
              }
              className="flex-row items-center gap-4 p-4 bg-card rounded-[12px]"
            >
              <Ionicons
