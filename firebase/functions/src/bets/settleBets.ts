 ...
 */
export const settleMarket = functions
  .region("us-central1")
  .https.onCall(async (data, context) => {
    ...
    if (!marketId || !result) {
      throw new functions.https.HttpsError(
        "invalid-argument",
        "Missing required fields: marketId, result",
      );
    }
    ...
    if (result !== "si" && result !== "no" && result !== "cancelled") {
      throw new functions.https.HttpsError(
        "invalid-argument",
        'Result must be "si", "no", or "cancelled"',
      );
    }
    ...
    try {
      ...
      const userBetsSnapshot = await db
        .collection("userBets")
        ...
        if (balanceChange !== 0) {
          ...
          if (userDoc.exists) {
            ...
            if (betStatus !== "lost") {
              ...
              console.log(
                `📝 Transaction created: ${transactionType} - $${balanceChange}`,
              );
            }
          }
        } else {
      ...
      // Execute all batches
      console.log(
        `🚀 Committing ${batches.length} batch(es) with ${settledCount} bet(s)`,
      );
      ...
      const userIds = Array.from(
        new Set(Array.from(betResults.values()).map((b) => b.userId)),
      );
      ...
      if (notificationCount > 0) {
        ...
        console.log(
          `📬 Created ${notificationCount} in-app notifications for market settlement`,
        );
      }
      ...
      for (const [_betId, betResult] of betResults.entries()) {
        const userData = userDataMap.get(betResult.userId);
      ...
      if (notificationMessages.length > 0) {
        ...
        for (let i = 0; i < notificationMessages.length; i += batchSize) {
          ...
          try {
            ...
            if (!response.ok) {
              ...
              console.log(
                `✅ Sent push notifications to ${messageBatch.length} devices (batch ${i / batchSize + 1})`,
                result,
              );
            }
          } catch (fetchError) {
        ...
        console.log(
          `📤 Sent push notifications to ${notificationMessages.length} users for market settlement ${marketId}`,
        );
      } else {
        console.log(
          `No push tokens available for market settlement ${marketId}`,
        );
      }

      console.log(
        `✅ Market ${marketId} settled with result: ${result}. Settled ${settledCount} bets.`,
      );
